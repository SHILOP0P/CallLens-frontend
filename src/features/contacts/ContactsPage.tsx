...
function ContactRow({ user, action, busy, onAction }: { user: UserResponse; action: "add" | "remove"; busy: boolean; onAction: () => void; }) {
  ...
  return <article className="contact-row"><span className="avatar">{name[0]?.toUpperCase() ?? "П"}</span><div><strong>{name}</strong><small>{user.username.startsWith("@") ? user.username : `@${user.username}`}{user.post ? ` · ${user.post}` : ""}</small></div><button className={action === "remove" ? "ghost-button small danger-button" : "primary-button small"} type="button" disabled={busy} onClick={onAction}>{action === "remove" ? <><UserMinus size={16} />Убрать</> : <><UserPlus size={16} />Добавить</>}</button></article>;
}
